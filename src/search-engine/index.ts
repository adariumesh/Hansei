...
import { ComponentRequest, DocumentToIndex } from './interfaces.js';
...
app.post('/api/index', async (c) => {
  try {
    const document = await c.req.json() as DocumentToIndex;
    if (!document || Object.keys(document).length === 0) {
      return c.json({ error: 'Document is required' }, 400);
    }
    const indexResult = await indexDocument(c.env, document);
...
app.post('/api/process', async (c) => {
  try {
    ...
    const response = await processRequest(c.env, request);
